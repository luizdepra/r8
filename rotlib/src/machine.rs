...
use crate::{
    keyboard::Keys,
    screen::{DEFAULT_SCREEN_HEIGHT, DEFAULT_SCREEN_WIDTH},
};
...
impl Machine {
    pub fn new() -> Self {
        ...
        ram[0..FONT_MEMORY_SIZE].copy_from_slice(&FONT);
    ...
    fn op_Dxyn(&mut self, x: u8, y: u8, n: u8) -> OperationResult {
        let x = (self.v[x as usize] as usize) % DEFAULT_SCREEN_WIDTH as usize;
        let y = (self.v[y as usize] as usize) % DEFAULT_SCREEN_HEIGHT as usize;
...
fn ram_index(x: usize, y: usize) -> Option<usize> {
    ...
    Some(y * DEFAULT_SCREEN_HEIGHT + x * DEFAULT_SCREEN_WIDTH)
}
